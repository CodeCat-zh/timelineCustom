...
{
    [CustomEditor(typeof(E_CutsceneImpulsePlayableAsset))]
    public class E_CutsceneImpulsePlayableInspector : BaseEditor<E_CutsceneImpulsePlayableAsset>
    {
...
#pragma warning disable 0649 // assigned but never used
        ...
        GUIContent mAnimatedLabel = new GUIContent("动画预览     ", "Animate the noise signal preview");
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.LabelField("频率，值越大，震动频率越高");
            EditorGUILayout.LabelField("    The frequency of noise for this channel.  Higher magnitudes vibrate faster");
            EditorGUILayout.Space();
            EditorGUILayout.LabelField("Amplitude :");
            EditorGUILayout.LabelField("振幅，值越大，震动的幅度越大");
            EditorGUILayout.LabelField("    The amplitude of the noise for this channel.  Larger numbers vibrate higher");
            EditorGUILayout.Space();
            ...
            mPreviewTime = EditorGUI.Slider(r, "预览框长度", mPreviewTime, 0.01f, 10f);
            ...
            mPreviewHeight = EditorGUI.Slider(r, "预览框高度", mPreviewHeight, 1f, 10f);
            r.x += r.width + hSpace; r.width = labelWidth;
            mAnimatedPreview = EditorGUI.ToggleLeft(r, mAnimatedLabel, mAnimatedPreview);
            EditorGUILayout.Separator();

            r = EditorGUILayout.GetControlRect();
            EditorGUI.LabelField(r, "位置震动预览-幅度以距离为单位", EditorStyles.boldLabel);
            ...
            r = EditorGUILayout.GetControlRect();
            EditorGUI.LabelField(r, "旋转震动预览-振幅单位为度", EditorStyles.boldLabel);
            r = EditorGUILayout.GetControlRect(true, mPreviewHeight * EditorGUIUtility.singleLineHeight);
            if (Event.current.type == EventType.Repaint)
            {
        ...
        class SampleCache
        {
            ...
            public void SnapshotSample(
                Vector2 areaSize, E_CutsceneImpulsePlayableAsset.TransformNoiseParams[] signal, float noiseOffset, bool animated)
            {
                ...
                for (int i = 0; i < numSamples; ++i)
                {
                    ...
                    Vector3 p = GetCombinedFilterResults(signal, t, Vector3.zero);
                    mSampleNoise.Add(p);
                }
                mSampleCurveX.Clear();
                mSampleCurveY.Clear();
            ...
            public Vector3 GetCombinedFilterResults(E_CutsceneImpulsePlayableAsset.TransformNoiseParams[] noiseParams, float time, Vector3 timeOffsets)
            {
        ...
        private ReorderableList SetupReorderableList(SerializedProperty property, int channel, GUIContent title)
        {
            ChannelList list = new ChannelList(property.serializedObject, property, channel, title);
